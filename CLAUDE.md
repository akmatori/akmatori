...
│       └── tools/          # SSH, Zabbix, VictoriaMetrics, Grafana, and HTTP connector implementations
├── web/                    # React frontend
...
| `list_tool_types` | `src/gateway-tools.ts` | List all available tool types (e.g., `ssh`, `zabbix`, `victoria_metrics`, `grafana`, `qmd`) |
| `list_tools_for_tool_type` | `src/gateway-tools.ts` | List all tools of a given type (e.g., `ssh`, `zabbix`, `victoria_metrics`, `grafana`) |
