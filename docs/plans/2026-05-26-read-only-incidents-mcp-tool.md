...
- [x] Define `IncidentsTool` struct with `db *gorm.DB` and `logger *log.Logger`
- [x] Implement `NewIncidentsTool(db *gorm.DB, logger *log.Logger) *IncidentsTool`
- [x] Implement `List(ctx, incidentID, args)`: parse optional `from`/`to` (unix timestamps), `status`, `source_kind`, `limit` (default 50, clamp to max 200), `offset`; GORM query with `Order("started_at DESC")`; return summary fields only (UUID, Title, Status, SourceKind, SourceUUID, StartedAt, CompletedAt, TokensUsed) as `{"incidents":[…],"count":N,"limit":L,"offset":O}`; ignore `incidentID`; use parameterized queries only
- [x] Implement `Get(ctx, incidentID, args)`: require `uuid` arg; fetch full Incident; truncate FullLog to 50,000 bytes if longer; return clean "not found" on `gorm.ErrRecordNotFound`; ignore `incidentID`
- [x] Write `incidents_test.go` using SQLite in-memory (`gorm.io/driver/sqlite`): open DB, auto-migrate `database.Incident`, insert fixtures
...
- [x] Import `incidents` package
- [x] Add `incidentsTool *incidents.IncidentsTool` field to `Registry` struct
- [x] Add `registerIncidentsTools()` method: construct `NewIncidentsTool(database.DB, r.logger)`, register `incidents.list` (all-optional schema: from, to, status, source_kind, limit, offset) and `incidents.get` (required uuid) with the server
- [x] Call `r.registerIncidentsTools()` in `RegisterAllTools()` (no rate limiter — local DB queries)
- [x] Add `"incidents": true` to `builtInToolNamespaces` map
- [x] Add tests to `registry_test.go` for `registerIncidentsTools`: two tools registered, correct tool names, correct required fields in schema
