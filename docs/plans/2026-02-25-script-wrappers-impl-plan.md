...
- [x] Write failing test `TestGenerateSkillMd_ContainsPythonExamples`:
...
- [x] Run test to verify it fails: `go test -v -run TestGenerateSkillMd_ContainsPythonExamples ./internal/services/...`
- [x] Update `TestGenerateSkillMd_NoPythonImports` (line 276): change to assert no OLD-style Python imports (`import sys`, `from scripts.`) but now EXPECTS `\`\`\`python` blocks
- [x] Extract `generateToolUsageExample(tool)` function with switch on tool type:
...
- [x] Update `generateSkillMd()` to call `generateToolUsageExample(tool)` for each enabled tool
- [x] Run all skill service tests: `go test -v -run "TestGenerateSkillMd|TestExtractToolDetails|TestStripAutoGenerated|TestAssignTools" ./internal/services/...` - all must pass
