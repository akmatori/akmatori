...
func (s *SkillService) GetSkillPrompt(name string) (string, error) {
	...
	if name == "cron-agent" {
		return database.DefaultCronAgentPrompt, nil
	}
...
`, logicalName, logicalName, logicalName)
	case "proposals":
		return fmt.Sprintf(`
**Parameters:**
- `+"`create`"+`: kind*, title*, proposed_content* (object) | reasoning, target_ref (required for *_update kinds), source_incident_uuids (array). Kinds: runbook_new, runbook_update, memory_new, memory_update, cron_new, cron_update, skill_prompt_update. proposed_content shapes — runbook: {title, content}; memory: {scope, type, name, description, body}; cron: {name, schedule, prompt, tool_logical_names?}; skill_prompt: {skill_name, prompt}.
- `+"`list`"+`: status (default "pending"), kind, include_content (bool), limit, offset
- `+"`get`"+`: uuid* (string)
- `+"`update_draft`"+`: uuid* | title, reasoning, proposed_content (object, full replacement in the per-kind shape). Pending proposals only.
- `+"`list_cron_jobs`"+`: (no parameters)
(* = required)

Usage (via gateway_call):
`+"```"+`
gateway_call("proposals.list", {"status": "pending"}, "%s")
gateway_call("proposals.create", {"kind": "runbook_new", "title": "Add runbook for nginx 502 storms", "reasoning": "...", "source_incident_uuids": ["<uuid>"], "proposed_content": {"title": "Nginx 502 storms", "content": "..."}}, "%s")
gateway_call("proposals.update_draft", {"uuid": "<proposal-uuid>", "proposed_content": {"title": "...", "content": "..."}}, "%s")
gateway_call("proposals.list_cron_jobs", {}, "%s")
`+"```"+`
