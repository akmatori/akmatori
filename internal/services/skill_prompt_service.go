...
// For incident-manager and cron-agent system skills, this is a no-op (the
// prompts are hardcoded). For regular skills, writes to SKILL.md file.
func (s *SkillService) UpdateSkillPrompt(name, prompt string) error {
	// System-skill prompts are hardcoded, can't be updated
	if name == "incident-manager" || name == "cron-agent" {
		return nil
	}
